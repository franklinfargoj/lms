...
/**
    ...
    */
     ...
     * @param boolean $calculateFormulas Should formulas be calculated?
     * @param boolean $formatData Should formatting be applied to cell values?
     * @param boolean $refkeys array of keys used as keys for cell value
     * @return array
     */
    /*public function rangeToArrayWithKeys($pRange = 'A1', $nullValue = null, $calculateFormulas = true, $formatData = true, $refkeys)
    {
        // Returnvalue
        $returnValue = array();
        //    Identify the range that we need to extract from the worksheet
        ...
        for ($row = $minRow; $row <= $maxRow; ++$row) {
            ...
            for ($col = $minCol; $col != $maxCol; ++$col) {
                ++$c;
                $cRef = (isset($refkeys[$c])) ? $refkeys[$c] : $col;
                //    Using getCell() will create a new cell if it doesn't already exist. We don't want that to happen
                //        so we test and retrieve directly against cellCollection
                if ($this->cellCollection->isDataSet($col.$row)) {
                    // Cell exists
                    ...
                    if ($cell->getValue() !== null) {
                        if ($cell->getValue() instanceof PHPExcel_RichText) {
                            $returnValue[$rRef][$cRef] = trim($cell->getValue()->getPlainText());
                        } else {
                            if ($calculateFormulas) {
                                $returnValue[$rRef][$cRef] = trim($cell->getCalculatedValue());
                            } else {
                                $returnValue[$rRef][$cRef] = trim($cell->getValue());
                            }
                        }
                        ...
                        // Cell holds a NULL
                        $returnValue[$rRef][$cRef] = trim($nullValue);
                    }
                } else {
                    // Cell doesn't exist
                    $returnValue[$rRef][$cRef] = trim($nullValue);
            ...
            if(empty(array_filter($returnValue[$rRef])))
            {
    ...
    public function rangeToArrayWithKeys($pRange = 'A1', $nullValue = null, $refkeys, $date_keys = null)
    {
        // Returnvalue
        $returnValue = array();
        //    Identify the range that we need to extract from the worksheet
        ...
        for ($row = $minRow; $row <= $maxRow; ++$row) {
            ...
            for ($col = $minCol; $col != $maxCol; ++$col) {
                ...
                if ($this->cellCollection->isDataSet($col.$row)) {
                    ...
                    if ($cell->getValue() !== null) {
                        $returnValue[$rRef][$cRef] = trim($cell->getCalculatedValue());
                        if(isset($date_keys) && in_array($cRef,$date_keys))
                        {
                            if(is_numeric($returnValue[$rRef][$cRef]))
                            {
                                $returnValue[$rRef][$cRef] = date('Y-m-d',ExcelDateToUnix($returnValue[$rRef][$cRef]));
                            }
                            else
                            {
