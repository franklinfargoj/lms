...
//        exit;
       ...
       if($action == 'status_flow'){
            $header_value = array('Sr. No.','Lead Id','Customer Name','Contact Number','Product Category','Product','Ticket Size','Lead Source','Lead Generation Date','Lead Generated By','Lead Generation Branch ID','HRMS ID of Lead Generator','Lead Assignment Date(1)','Lead Assigned To (1)','Lead Assigned to HRMS ID (1)','Lead Assigned To Branch ID (1)','Lead Assigned To Branch Name (1)','Lead Assignment Date(2)','Lead Assigned To (2)','Lead Assigned to HRMS ID (2)','Lead Assigned To Branch ID (2)','Lead Assigned To Branch Name (2)','Lead Assignment Date(3)','Lead Assigned To (3)','Lead Assigned to HRMS ID (3)','Lead Assigned to Branch ID (3)','Lead Assigned to Branch Name (3)','Lead Marked as Interested Date','Lead Marked as Interested By','Lead Marked as Interested By HRMS ID','Lead Identified As','Lead Marked as Documents Collected Date','Lead Marked as Documents Collected By','Lead Marked as Documents Collected By HRMS ID','Lead Marked as Account Opened Date','Lead Marked as Account Opened By','Lead Marked as Account Opened By HRMS ID','Lead marked as Drop/Not Interested Date','Lead marked as Drop/Not Interested By','Lead marked as Drop/Not Interested By HRMS ID','Reason for Drop','Lead Marked as Converted Date','Lead Marked as Converted By','Lead Marked as Converted By HRMS ID','Account Number of lead converted','Account Balance of Lead Converted','Lead marked as Reject Date','Lead marked as Reject By','Lead marked as Reject By HRMS ID');
        }
...
//        }
        ...
        foreach($arrData['generated_leads'] as $row){
            ...
            $final_result = array();
            $action = 'list';
            $select = array('l.id','l.customer_name','l.lead_source','l.contact_no','l.created_by_branch_id','l.created_on AS generated_on','l.reroute_from_branch_id','l.modified_on','l.branch_id','l.created_by_name as generated','rfc.amount');
            $table = Tbl_Leads.' as l';
            $join = array();
        ...
        foreach ($data['status_flow'] as $key => $value) {
            ...
            foreach ($value as $key => $value1) {
                if($key == 0) {
                    ...
                    $objSheet->getCell($excel_alpha[11] . $i)->setValue(ucwords($value[$key]['created_by_hrms_id']));
                ...
                if($key > 0){
                    ...
                    if(isset($value[0]['reroute_from_branch_id'])) {
                        if ($value[$key]['status'] == 'NC' && $value[$key]['assignee_branch'] == $value[0]['reroute_from_branch_id']) {
                            $objSheet->getCell($excel_alpha[12] . $i)->setValue(ucwords($value[$key]['date']));
                            $objSheet->getCell($excel_alpha[13] . $i)->setValue(ucwords($value[$key]['employee_name']));
                            $objSheet->getCell($excel_alpha[14] . $i)->setValue(ucwords($value[$key]['employee_id']));
                        }
                        if ($value[$key]['status'] == 'NC' && $value[$key]['assignee_branch'] == $value[0]['branch_id']) {
                            $objSheet->getCell($excel_alpha[17] . $i)->setValue(ucwords($value[$key]['date']));
                            $objSheet->getCell($excel_alpha[18] . $i)->setValue(ucwords($value[$key]['employee_name']));
                            $objSheet->getCell($excel_alpha[19] . $i)->setValue(ucwords($value[$key]['employee_id']));
                        }
                    }else{
                        $objSheet->getCell($excel_alpha[12] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[13] . $i)->setValue(ucwords($value[$key]['employee_name']));
                    ...
                    if($value[$key]['status'] == 'FU'){
                        $objSheet->getCell($excel_alpha[27] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[28] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[29] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }

                    $objSheet->getCell($excel_alpha[30] . $i)->setValue(ucwords($lead_type[$value[0]['lead_identification']]));

                    if($value[$key]['status'] == 'DC'){
                        $objSheet->getCell($excel_alpha[31] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[32] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[33] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }

                    if($value[$key]['status'] == 'AO'){
                        $objSheet->getCell($excel_alpha[34] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[35] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[36] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }

                    if($value[$key]['status'] == 'NI'){
                        $objSheet->getCell($excel_alpha[37] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[38] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[39] . $i)->setValue(ucwords($value[$key]['modified_by']));
                        $objSheet->getCell($excel_alpha[40] . $i)->setValue(ucwords($value[$key]['reason_for_drop']));
                    }

                    if($value[$key]['status'] == 'Converted'){
                        $objSheet->getCell($excel_alpha[41] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[42] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[43] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    ...
                    if($value[$key]['status'] == 'Closed'){
                        $objSheet->getCell($excel_alpha[46] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[47] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[48] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }
