    ...
    */
    ...
    public function unassigned_leads_post()
    {
        ...
        if (!empty($params) && isset($params['branch_id']) && !empty($params['branch_id'])) {
            ...
            $where = array(Tbl_Leads . '.branch_id' => $params['branch_id'], 'YEAR(' . Tbl_Leads . '.created_on)' => date('Y'), 'DATEDIFF( CURDATE( ) , ' . Tbl_Leads . '.created_on) <=' => Elapsed_day);
...
$arrData['unassigned_leads_count'] = $this->Lead->unassigned_status_count($select, $table, $join, $where, $group_by);
    ...
    public function lead_details_post()
    {
        ...
        if (!empty($params) && isset($params['lead_id']) && !empty($params['lead_id']) && isset($params['type']) && !empty($params['type'])) {
            ...
            if ($type == 'generated') {
                $select = array('l.id', 'l.customer_name', 'l.lead_identification','l.reroute_from_branch_id','l.opened_account_no', 'l.lead_source', 'l.contact_no', 'l.product_id','l.created_by_branch_id', 'p.title AS product_title','p.map_with', 'c.title AS category_title', 'l.product_category_id', 'la.status','la.desc_for_drop', 'l.remark');
...
//            $order_by = "n.priority ASC";
            ...
            if ($result['basic_info']['designation'] == 'BM') {
                ...
                $select = array();
                $table = Tbl_Leads;
                $where = array(Tbl_Leads . '.branch_id' => $result['basic_info']['branch_id'], 'YEAR(' . Tbl_Leads . '.created_on)' => date('Y'), 'DATEDIFF( CURDATE( ) , ' . Tbl_Leads . '.created_on) <=' => Elapsed_day);
$where['('.Tbl_LeadAssign.'.lead_id IS NULL OR '.Tbl_LeadAssign.'.is_deleted = 1)'] = NULL;
