...
//        exit;
       ...
       if($action == 'status_flow'){
            $header_value = array('Sr. No.','Lead Id','Customer Name','Contact Number','Product Category','Product','Ticket Size','Lead Source','Lead Generation Date','Lead Generated By','Lead Generation Branch ID','Lead Generation Branch Name','Lead Generation Zone Name','Lead Assigned Current Branch Name','HRMS ID of Lead Generator','Lead Assignment Date(1)','Lead Assigned To (1)','Lead Assigned to HRMS ID (1)','Lead Assigned To Branch ID (1)','Lead Assigned To Branch Name (1)','Lead Assignment Date(2)','Lead Assigned To (2)','Lead Assigned to HRMS ID (2)','Lead Assigned To Branch ID (2)','Lead Assigned To Branch Name (2)','Lead Marked as Interested Date','Lead Marked as Interested By','Lead Marked as Interested By HRMS ID','Lead Identified As','Lead Marked as Documents Collected Date','Lead Marked as Documents Collected By','Lead Marked as Documents Collected By HRMS ID','Lead Marked as Account Opened Date','Lead Marked as Account Opened By','Lead Marked as Account Opened By HRMS ID','Lead marked as Drop/Not Interested Date','Lead marked as Drop/Not Interested By','Lead marked as Drop/Not Interested By HRMS ID','Reason for Drop','Lead Marked as Converted Date','Lead Marked as Converted By','Lead Marked as Converted By HRMS ID','Account Number of lead converted','Account Balance of Lead Converted','Lead marked as Reject Date','Lead marked as Reject By','Lead marked as Reject By HRMS ID');
        }
...
//        }
        ...
        foreach($arrData['generated_leads'] as $row){
            ...
            $final_result = array();
            $action = 'list';
            $select = array('l.id','l.customer_name','l.lead_source','l.contact_no','l.created_by_branch_id','l.created_by_zone_id','l.created_on AS generated_on','l.reroute_from_branch_id','l.modified_on','l.branch_id','l.created_by_name as generated','rfc.amount','l.opened_account_no');
            $table = Tbl_Leads.' as l';
            $join = array();
        ...
        foreach ($data['status_flow'] as $key => $value) {
            ...
            foreach ($value as $key => $value1) {
                if($key == 0) {
                    ...
                    $objSheet->getCell($excel_alpha[10] . $i)->setValue(ucwords($value[$key]['created_by_branch_id']));
                    $branchname11 = branchname($value[$key]['created_by_branch_id']);
                    $branchname11 = $branchname11[0]['name'];
                    $objSheet->getCell($excel_alpha[11] . $i)->setValue(ucwords($branchname11));
                    $zonename12 = zonename($value[$key]['created_by_zone_id']);
                    $zonename12 = $zonename12[0]['name'];
                    $objSheet->getCell($excel_alpha[12] . $i)->setValue(ucwords($zonename12));
                    $branchname13 = branchname($value[$key]['branch_id']);
                    $branchname13 = $branchname13[0]['name'];
                    $objSheet->getCell($excel_alpha[13] . $i)->setValue(ucwords($branchname13));
                    $objSheet->getCell($excel_alpha[14] . $i)->setValue(ucwords($value[$key]['created_by_hrms_id']));
                ...
                if($key > 0){
                    ...
                    if(isset($value[0]['reroute_from_branch_id'])) {
                        if ($value[$key]['status'] == 'NC' && $value[$key]['assignee_branch'] == $value[0]['reroute_from_branch_id']) {
                            $objSheet->getCell($excel_alpha[15] . $i)->setValue(ucwords($value[$key]['date']));
                            $objSheet->getCell($excel_alpha[16] . $i)->setValue(ucwords($value[$key]['employee_name']));
                            $objSheet->getCell($excel_alpha[17] . $i)->setValue(ucwords($value[$key]['employee_id']));
                            $objSheet->getCell($excel_alpha[18] . $i)->setValue(ucwords($value[0]['reroute_from_branch_id']));
                            $branchname1 = branchname($value[0]['reroute_from_branch_id']);
                            $branchname1 = $branchname1[0]['name'];
                            $objSheet->getCell($excel_alpha[19] . $i)->setValue(ucwords($branchname1));

                           
                        }
                        if ($value[$key]['status'] == 'NC' && $value[$key]['assignee_branch'] == $value[0]['branch_id']) {
                            $objSheet->getCell($excel_alpha[20] . $i)->setValue(ucwords($value[$key]['date']));
                            $objSheet->getCell($excel_alpha[21] . $i)->setValue(ucwords($value[$key]['employee_name']));
                            $objSheet->getCell($excel_alpha[22] . $i)->setValue(ucwords($value[$key]['employee_id']));
                            $objSheet->getCell($excel_alpha[23] . $i)->setValue(ucwords($value[0]['branch_id']));
                            $branchname2 = branchname($value[0]['branch_id']);
                            $branchname2 = $branchname2[0]['name'];
                            $objSheet->getCell($excel_alpha[24] . $i)->setValue(ucwords($branchname2));
                        }
                    }else{
                        $objSheet->getCell($excel_alpha[15] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[16] . $i)->setValue(ucwords($value[$key]['employee_name']));
                        $objSheet->getCell($excel_alpha[17] . $i)->setValue(ucwords($value[$key]['employee_id']));
                        $objSheet->getCell($excel_alpha[18] . $i)->setValue(ucwords($value[0]['branch_id']));
                    ...
                    if($value[$key]['status'] == 'FU'){
                        $objSheet->getCell($excel_alpha[25] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[26] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[27] . $i)->setValue(ucwords($value[$key]['modified_by']));
                        $objSheet->getCell($excel_alpha[28] . $i)->setValue(ucwords($lead_type[$value[0]['lead_identification']]));
                    }

                    

                    if($value[$key]['status'] == 'DC'){
                        $objSheet->getCell($excel_alpha[29] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[30] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[31] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }

                    if($value[$key]['status'] == 'AO'){
                        $objSheet->getCell($excel_alpha[32] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[33] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[34] . $i)->setValue(ucwords($value[$key]['modified_by']));
                        $objSheet->getCell($excel_alpha[42] . $i)->setValue(ucwords($value[0]['opened_account_no']));
                        $objSheet->getCell($excel_alpha[43] . $i)->setValue(ucwords($value[0]['amount']));
                    }

                    if($value[$key]['status'] == 'NI'){
                        $objSheet->getCell($excel_alpha[35] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[36] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[37] . $i)->setValue(ucwords($value[$key]['modified_by']));
                        $objSheet->getCell($excel_alpha[38] . $i)->setValue(ucwords($value[$key]['reason_for_drop']));
                    }

                    if($value[$key]['status'] == 'Converted'){
                        $objSheet->getCell($excel_alpha[39] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[40] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[41] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }

                    if($value[$key]['status'] == 'Closed'){
                        $objSheet->getCell($excel_alpha[44] . $i)->setValue(ucwords($value[$key]['date']));
                        $objSheet->getCell($excel_alpha[45] . $i)->setValue(ucwords($value[$key]['modified_by_name']));
                        $objSheet->getCell($excel_alpha[46] . $i)->setValue(ucwords($value[$key]['modified_by']));
                    }
